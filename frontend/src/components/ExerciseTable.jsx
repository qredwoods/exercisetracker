...
const ExerciseTable = ({user, exercises, onDelete, onEdit, onDuplicate, onView, isFirstVisit, fadeIn, onFadeComplete, highlightId, onHighlightEnd, deletingId}) => {
  ...
  const pendingMoreScrollRef = useRef(null);
  ...
  useEffect(() => {
    const pendingScroll = pendingMoreScrollRef.current;
    const frame = tableFrameRef.current;
    if (!pendingScroll || !frame) return;

    const heightAfter = frame.getBoundingClientRect().height;
    const delta = heightAfter - pendingScroll.heightBefore;
    if (delta > 0) {
      window.scrollBy({ top: delta, behavior: "smooth" });
    }
    pendingMoreScrollRef.current = null;
  }, [visibleCount]);
  ...
  const showFadeTail = hasMoreExercises;
  const showBelowTableSummary = !hasMoreExercises && visibleCount > DEFAULT_VISIBLE_COUNT;

  const handleShowMore = () => {
    const frame = tableFrameRef.current;
    pendingMoreScrollRef.current = frame
      ? { heightBefore: frame.getBoundingClientRect().height }
      : null;
    setVisibleCount((count) => count + DEFAULT_VISIBLE_COUNT);
  };
    ...
    <div>
      ...
      <div className={`table-scaler${fadeIn ? " fade-in" : ""}${showTableOverlay ? " table-scaler--overlay" : ""}`} onAnimationEnd={onFadeComplete}>
        <div className="table-frame" ref={tableFrameRef}>
          <div className={`table-mask${showFadeTail ? " table-mask--fade" : ""}`}>
            <table className={showFadeTail ? "table--fade-tail" : ""}>
              <thead>
                <tr>
                  <th
              ...
              {isEmpty ? (
                ...
                renderedExercises.map((exercise) => (
                  <ExerciseRow
                    ...
                    onView={onView}
                    highlight={exercise._id === highlightId}
                    onHighlightEnd={onHighlightEnd}
                    deleting={exercise._id === deletingId}
          ...
          {showFadeTail && (
            <div className={`table-overlay${showFadeTail ? " table-overlay--fade" : " table-overlay--compact"}`}>
              <div className="table-overlay-stack">
                ...
                <p className="table-subheading table-subheading--overlay">
      ...
      {!isEmpty && !showTableOverlay && (
        <p className="table-subheading">
          Showing {Math.min(visibleCount, exercises.length)} of {exercises.length}
        </p>
      )}
      {showBelowTableSummary && (
        <div className="table-below-controls">
          <button
            className="table-more-button"
            onClick={() => setVisibleCount(DEFAULT_VISIBLE_COUNT)}
          >
          ...
          <p className="table-subheading table-subheading--below">
            Showing {Math.min(visibleCount, exercises.length)} of {exercises.length}
          </p>
        </div>
      )}
